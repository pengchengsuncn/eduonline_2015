...
<form name="form" id="form" method="post" action="member_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		<tr align="left" class="dataTr">
            ...
            <td><?php if($row['is_teacher']=='true') echo '是';else echo '否'; ?></td>
			<td class="action endCol"><span><a href="member_recommend.php?id=<?php echo $row['id']; ?>"><?php $info=$dosql->GetOne("SELECT * FROM `#@__member_recommend` WHERE `uid`=".$row['id']); if(isset($info['id'])) echo '取消推荐';elseif($row['is_teacher']=='true') echo '推荐';?></a></span> | <span><a href="member_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span class="nb"><a href="member_save.php?action=del2&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0)">删除</a></span></td>
		</tr>
		<?php
		}
		?>
